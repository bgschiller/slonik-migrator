...
export class SlonikMigrator extends umzug.Umzug<SlonikMigratorContext> {
  constructor(
    ...
    super({
      ...
      migrations: () => ({
        glob: [
          this.migrationsGlob(),
          { cwd: path.resolve(slonikMigratorOptions.migrationsPath) },
        ],
  ...
  protected advisoryLockId() {
    const hashable =
      "@slonik/migrator advisory lock:" +
      JSON.stringify(this.slonikMigratorOptions.migrationTableName);
  ...
  protected resolver(
    ...
    return {
      ...
      path: params.path,
      up: async (upParams) =>
        import(params.path!).then((mod) =>
          mod.up({ slonik, sql, ...upParams })
        ),
      down: async (downParams) =>
        import(params.path!).then((mod) =>
          mod.down?.({ slonik, sql, ...downParams })
        ),
    };
  }
  ...
  async runCommand<T>(
    command: string,
    cb: (params: { context: SlonikMigratorContext }) => Promise<T>
  ) {
    let run = cb;
    if (command === "up" || command === "down") {
      run = async ({ context }) => {
        return context.parent.connect(async (conn) => {
          ...
          await conn.any(
            context.sql
              .unsafe`select pg_advisory_lock(${this.advisoryLockId()})`
          );
          ...
          try {
            ...
            await conn
              .any(
                context.sql
                  .unsafe`select pg_advisory_unlock(${this.advisoryLockId()})`
              )
              .catch((error) => {
    ...
    return super.runCommand(command, async ({ context: _ctx }) => {
      const connect = this.slonikMigratorOptions.singleTransaction
        ? _ctx.parent.transaction
        : _ctx.parent.connect;
  ...
  async repair(options?: RepairOptions) {
    ...
    await this.runCommand("repair", async ({ context }) => {
      ...
      const migrationsThatNeedRepair = infos.filter(
        ({ dbHash, diskHash }) => dbHash !== diskHash
      );
  ...
  protected hash(name: string) {
    return createHash("md5")
      .update(
        readFileSync(
          join(this.slonikMigratorOptions.migrationsPath, name),
          "utf8"
        )
          .trim()
          .replace(/\s+/g, " ")
  ...
  protected async unlogMigration({
    name,
    context,
  }: {
    name: string;
...
export type Migration = (
  params: umzug.MigrationParams<SlonikMigratorContext> & {
    ...
    /** @deprecated use `context.sql.unsafe` */
    sql: typeof sql;
  }
) => Promise<unknown>;
...
export const setupSlonikMigrator = (
  ...
  const migrator = new SlonikMigrator({
    ...
    migrationTableName:
      options.migrationTableName || defaultMigrationTableName(),
...
const createMessageFormats = <
  T extends Record<string, (msg: LogMessage) => [string, LogMessage]>
>(
  formats: T
) => formats;
...
function prettifyAndLog(
  level: keyof typeof SlonikMigrator.prettyLogger,
  message: LogMessage
) {
  const { event } = message || {};
