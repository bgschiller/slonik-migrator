...
describe("locking", async () => {
  ...
  test("second instance waits for first", async () => {
    ...
    const syncer = fsSyncer(baseDir, {
      migrations: {
        ...
        "m2.sql":
          "select pg_sleep(3); insert into locking_test_table(id) values (1);", // runs slowly, and fails if it's run twice
        down: {
    ...
    expect(await migrator().executed().then(names)).toEqual([
      "m1.sql",
      "m2.sql",
    ]);
  });
  ...
  test(`singleTransaction doesn't prevent unlock`, async () => {
    ...
    await expect(migrator.up()).rejects.toThrowErrorMatchingInlineSnapshot(
      `[MigrationError: Migration m2.sql (up) failed: Original error: syntax error at or near "this"]`
    );
...
describe("concurrency", async () => {
  const helper = await getPoolHelper({
    __filename,
    config: {
      interceptors: [
        {
          afterPoolConnection: async (context, connection) => {
            await connection.query(sql.unsafe`set lock_timeout = '1s'`);
            return null;
          },
        },
      ],
  ...
  test("local lock timeout", async () => {
    ...
    const syncer = fsSyncer(baseDir, {
      migrations: {
        ...
        "m2.sql":
          "select pg_sleep(3); insert into lock_timeout_test_table(id) values (1);", // runs slowly, and fails if it's run twice
        down: {
