...
describe("run migrations", async () => {
  ...
  test("migrationTableName array format", async () => {
    await pool.query(
      sql.unknown`drop schema if exists some_other_schema cascade`
    );
    await pool.query(sql.unknown`create schema some_other_schema`);
    ...
    expect(await migrator.executed().then(helper.names)).toEqual([
      "01.one.sql",
    ]);

    expect(
      await pool.manyFirst(
        sql.unknown`select name from some_other_schema.migration_meta`
      )
    ).toEqual(["01.one.sql"]);
  });
});
